...
class ListViewTest(TestCase):
    ...
    def test_uses_list_template(self):
        ...
        response = self.client.get(f'/carolslist/{list_.id}/')
    ...
    def test_passes_correct_list_to_template(self):       
        ...
        response = self.client.get(f'/carolslist/{correct_list.id}/')
        self.assertEqual(response.context['list'], correct_list)  
 
class NewListTest(TestCase):   

    def test_redirects_after_POST(self):        
        response = self.client.post('/carolslist/new', data={'Name': 'MyNames','School': 'YourSchool','Precinct':'PrecinctId','NxtName': 'othername','Year': 'MySchoolYear','GPA': 'Mygrade'})                     
        new_list = List.objects.first()        
        self.assertRedirects(response, f'/carolslist/{new_list.id}/')
